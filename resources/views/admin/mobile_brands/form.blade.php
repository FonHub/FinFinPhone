        ...
        <div class="wrapper-box">
            ...
            <div class="content">
                ...
                <div class="intro-y box p-5">
                    ...
                    @endif
                    <form method="POST"
                        action="{{ $mode === 'edit' ? url('admin/mobile-brands/' . $brand->id . '/update') : url('admin/mobile-brands/store') }}">
                        @csrf
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 md:col-span-6">
                                <label class="form-label">ชื่อแบรนด์ <span class="text-danger">*</span></label>
                                <input type="text" name="name" class="form-control"
                                    value="{{ old('name', $brand->name ?? '') }}" placeholder="กรอกชื่อแบรนด์สินค้า">
                            </div>
                            ...
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <select name="status" class="form-select">
                                    <option value="1"
                                        {{ old('status', isset($brand->status) ? (int) $brand->status : 1) == 1 ? 'selected' : '' }}>
                                        เปิดใช้งาน
                                    </option>
                                    <option value="0"
                                        {{ old('status', isset($brand->status) ? (int) $brand->status : 1) == 0 ? 'selected' : '' }}>
                                        ปิดใช้งาน
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="mt-5 flex items-center">
                            <a href="{{ url('admin/mobile-brands') }}" class="btn btn-outline-secondary w-24 mr-2">
                                ยกเลิก
                            </a>

                            <button type="submit" class="btn btn-primary w-24">
                                {{ $mode === 'edit' ? 'บันทึก' : 'เพิ่มข้อมูล' }}
                            </button>
                        </div>
                    </form>
                </div>
